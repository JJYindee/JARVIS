...
from PyQt6.QtWidgets import (
    QApplication, QComboBox, QCheckBox, QFileDialog, QFormLayout, QFrame, QGraphicsOpacityEffect,
...
class ToolProgressWidget(QWidget):
    """Bottom Activity & Arrow Panel Bar.
    Shows real-time status animation (Listening, Thinking, Working, Speaking),
    active tool execution, elapsed timer, live wave/spinner animation,
    and features an arrow toggle button (▲/▼) to expand/collapse detailed activity logs.
    """
    ...
    def __init__(self, parent=None):
        ...
        self._show_sig.connect(self._on_show_tool)
        self._hide_sig.connect(self._on_hide_tool)
        self._state_sig.connect(self._on_state_change)
        self._log_sig.connect(self._on_append_log)
    ...
    def _build_ui(self):
        ...
        self._bar.mousePressEvent = lambda _: self.toggle_expand()

        bar_lay = QHBoxLayout(self._bar)
        bar_lay.setContentsMargins(12, 2, 12, 2)
        bar_lay.setSpacing(8)

        # LED status dot
        self._led = QLabel("●")
        self._led.setFont(QFont("Segoe UI", 11, QFont.Weight.Bold))
        self._led.setStyleSheet(f"color: {C.PRI}; background: transparent; border: none;")
        bar_lay.addWidget(self._led)

        # Animated indicator (spinner / wave)
        self._anim_lbl = QLabel("●")
        self._anim_lbl.setFont(QFont("Courier New", 10, QFont.Weight.Bold))
        self._anim_lbl.setStyleSheet(f"color: {C.ENERGY}; background: transparent; border: none;")
        bar_lay.addWidget(self._anim_lbl)

        # Main status text
        self._status_lbl = QLabel("⚡ [ONLINE] JARVIS พร้อมรับคำสั่ง")
        self._status_lbl.setFont(QFont(UI_FONT, 9, QFont.Weight.DemiBold))
        self._status_lbl.setStyleSheet(f"color: {C.WHITE}; background: transparent; border: none;")
        bar_lay.addWidget(self._status_lbl, stretch=1)

        # Elapsed timer
        self._time_lbl = QLabel("")
        self._time_lbl.setFont(QFont("Courier New", 8))
        self._time_lbl.setStyleSheet(f"color: {C.TEXT_DIM}; background: transparent; border: none;")
        bar_lay.addWidget(self._time_lbl)

        # Remaining Token Badge
        self._token_chip_lbl = QLabel("🎯 1.05M TOKENS")
        self._token_chip_lbl.setFont(QFont("Courier New", 8, QFont.Weight.Bold))
        self._token_chip_lbl.setStyleSheet(f"color: {C.GREEN}; background: {C.DARK}; border: 1px solid {C.BORDER}; border-radius: 4px; padding: 2px 6px;")
        bar_lay.addWidget(self._token_chip_lbl)

        # Arrow toggle button (▲ / ▼)
        self._arrow_btn = QPushButton("▲")
        self._arrow_btn.setFixedSize(24, 24)
        self._arrow_btn.setFont(QFont("Segoe UI", 9, QFont.Weight.Bold))
        ...
        bar_lay.addWidget(self._arrow_btn)

        main_lay.addWidget(self._bar)

        # ── Expanded Drawer Panel (collapsible logs & activity) ──────────────
        self._drawer = QFrame(self)
        self._drawer.setFixedHeight(110)
        self._drawer.hide()

        drawer_lay = QVBoxLayout(self._drawer)
        drawer_lay.setContentsMargins(10, 6, 10, 6)
        drawer_lay.setSpacing(4)

        drawer_hdr = QLabel("ACTIVITY & TOOL LOGS  /  บันทึกการทำงานและเครื่องมือ")
        drawer_hdr.setFont(QFont(UI_FONT, 8, QFont.Weight.Bold))
        drawer_hdr.setStyleSheet(f"color: {C.PRI}; background: transparent; border: none; letter-spacing: 1px;")
        drawer_lay.addWidget(drawer_hdr)
        ...
        """)
        drawer_lay.addWidget(self._log_view, stretch=1)

        main_lay.addWidget(self._drawer)
    ...
    def _update_bar_style(self):
        ...
        if self._active_tool:
            bg_col = f"{C.PURPLE}22"
            border_col = f"{C.PURPLE}66"
        elif self._current_state in ("LISTENING", "SPEAKING"):
            bg_col = f"{C.PRI}22"
            border_col = f"{C.PRI}66"
        elif self._current_state in ("THINKING", "PROCESSING", "WORKING"):
            bg_col = f"{C.ACC2}22"
            border_col = f"{C.ACC2}66"
    ...
    def set_state_status(self, state: str):
    ...
    def append_activity_log(self, text: str):
        self._log_sig.emit(text)

    def _on_state_change(self, state: str):
        self._current_state = state
        self._update_display()
        self.append_activity_log(f"🔄 [STATE] {state}")

    def _on_show_tool(self, name: str):
        self._active_tool = name
        self._start_time = time.time()
        self._update_display()
        self.append_activity_log(f"⚡ [TOOL] Executing: {name}")

    def _on_hide_tool(self):
        if self._active_tool:
            self.append_activity_log(f"✅ [TOOL] Finished: {self._active_tool}")
        self._active_tool = ""
        self._update_display()
    ...
    def _update_display(self):
        ...
        if self._active_tool:
            self._led.setStyleSheet(f"color: {C.PURPLE}; background: transparent; border: none;")
            self._status_lbl.setText(f"⚡ [WORKING] {self._active_tool.upper()}  /  กำลังทำงาน...")
        elif self._current_state == "LISTENING":
            self._led.setStyleSheet(f"color: {C.GREEN}; background: transparent; border: none;")
            self._status_lbl.setText("🎙️ [LISTENING] กำลังฟัง... (Listening)")
        elif self._current_state in ("THINKING", "PROCESSING"):
            self._led.setStyleSheet(f"color: {C.ACC2}; background: transparent; border: none;")
            self._status_lbl.setText("🧠 [THINKING] กำลังประมวลผล... (Processing)")
        elif self._current_state == "SPEAKING":
            self._led.setStyleSheet(f"color: {C.PRI}; background: transparent; border: none;")
            self._status_lbl.setText("🔊 [SPEAKING] กำลังพูดโต้ตอบ... (Speaking)")
        elif self._current_state == "MUTED":
            self._led.setStyleSheet(f"color: {C.RED}; background: transparent; border: none;")
            self._status_lbl.setText("🔇 [MUTED] ปิดไมโครโฟน")
        else:
            self._led.setStyleSheet(f"color: {C.PRI}; background: transparent; border: none;")
            self._status_lbl.setText("⚡ [ONLINE] JARVIS พร้อมรับคำสั่ง")

    def _tick(self):
        self._anim_idx = (self._anim_idx + 1) % len(self._spinner_chars)
        
        if self._current_state == "SPEAKING":
            w_idx = self._anim_idx % len(self._wave_chars)
            self._anim_lbl.setText(self._wave_chars[w_idx])
            self._anim_lbl.setStyleSheet(f"color: {C.PRI}; background: transparent; border: none;")
        elif self._current_state in ("THINKING", "PROCESSING") or self._active_tool:
            self._anim_lbl.setText(self._spinner_chars[self._anim_idx])
            self._anim_lbl.setStyleSheet(f"color: {C.ACC2 if not self._active_tool else C.PURPLE}; background: transparent; border: none;")
        elif self._current_state == "LISTENING":
            w_idx = self._anim_idx % len(self._wave_chars)
            self._anim_lbl.setText(self._wave_chars[w_idx])
            self._anim_lbl.setStyleSheet(f"color: {C.GREEN}; background: transparent; border: none;")
        else:
            self._anim_lbl.setText("●")
            self._anim_lbl.setStyleSheet(f"color: {C.PRI}; background: transparent; border: none;")

        if self._active_tool and self._start_time > 0:
...
# ---------------------------------------------------------------------------
# TokenMeterWidget — Gemini context token meter & remaining token gauge
# ---------------------------------------------------------------------------

class TokenMeterWidget(QFrame):
    """Token Usage & Remaining Context Tokens Display Widget.
    Shows used tokens, remaining context capacity (out of 1.05M tokens),
    and a visual capacity progress bar.
    """

    _update_sig = pyqtSignal(int, int)

    def __init__(self, parent=None, max_tokens: int = 1048576):
        super().__init__(parent)
        ...
        # Header row
        hdr_row = QHBoxLayout()
        hdr_lbl = QLabel("🎯 CONTEXT TOKENS / โทเคนคงเหลือ")
        hdr_lbl.setFont(QFont(UI_FONT, 8, QFont.Weight.Bold))
        hdr_lbl.setStyleSheet(f"color: {C.PRI}; background: transparent; border: none; letter-spacing: 1px;")
        hdr_row.addWidget(hdr_lbl)
        hdr_row.addStretch()

        self._pct_lbl = QLabel("100.0% REMAINING")
        self._pct_lbl.setFont(QFont("Courier New", 8, QFont.Weight.Bold))
        self._pct_lbl.setStyleSheet(f"color: {C.GREEN}; background: transparent; border: none;")
        hdr_row.addWidget(self._pct_lbl)

        lay.addLayout(hdr_row)

        # Remaining Token Count Big Display
        self._count_lbl = QLabel("1,048,576 Tokens คงเหลือ")
        self._count_lbl.setFont(QFont(UI_FONT, 9, QFont.Weight.DemiBold))
        self._count_lbl.setStyleSheet(f"color: {C.WHITE}; background: transparent; border: none;")
        ...
        self._bar.setStyleSheet(f"""
            ...
            QProgressBar::chunk {{
                background: qlineargradient(x1:0, y1:0, x2:1, y2:0,
                    stop:0 {C.GREEN}, stop:0.8 {C.PRI}, stop:1 {C.ACC2});
                border-radius: 3px;
            }}
        """)
        ...
        self._used_lbl = QLabel("Used: 0 Tokens")
        self._used_lbl.setFont(QFont("Courier New", 7))
        self._used_lbl.setStyleSheet(f"color: {C.TEXT_DIM}; background: transparent; border: none;")
        detail_row.addWidget(self._used_lbl)

        detail_row.addStretch()

        self._max_lbl = QLabel("Max: 1.05M Tokens")
        self._max_lbl.setFont(QFont("Courier New", 7))
        self._max_lbl.setStyleSheet(f"color: {C.TEXT_DIM}; background: transparent; border: none;")
        detail_row.addWidget(self._max_lbl)

        lay.addLayout(detail_row)

        self._update_sig.connect(self._on_update)

    def set_token_usage(self, used_tokens: int, max_tokens: int = 1048576):
        self._update_sig.emit(used_tokens, max_tokens)

    def _on_update(self, used_tokens: int, max_tokens: int):
        if max_tokens and max_tokens > 0:
            self._max_tokens = max_tokens
        self._used_tokens = max(0, int(used_tokens))
        remaining = max(0, self._max_tokens - self._used_tokens)
        pct_used = min(100.0, (self._used_tokens / self._max_tokens) * 100.0) if self._max_tokens > 0 else 0
        pct_rem = max(0.0, 100.0 - pct_used)

        self._bar.setValue(int(pct_used))
        self._count_lbl.setText(f"{remaining:,} / {self._max_tokens:,} Tokens คงเหลือ")
        self._used_lbl.setText(f"Used: {self._used_tokens:,} ({pct_used:.1f}%)")
        
        if pct_rem > 50:
            self._pct_lbl.setStyleSheet(f"color: {C.GREEN}; background: transparent; border: none;")
        elif pct_rem > 20:
            self._pct_lbl.setStyleSheet(f"color: {C.ACC2}; background: transparent; border: none;")
        else:
            self._pct_lbl.setStyleSheet(f"color: {C.RED}; background: transparent; border: none;")
        self._pct_lbl.setText(f"{pct_rem:.1f}% REMAINING")


# ---------------------------------------------------------------------------
# GeminiApiKeyWidget — interactive API key management panel
# ---------------------------------------------------------------------------

class GeminiApiKeyWidget(QFrame):
    """Interactive Gemini API Key Editor & Validator Widget.
    Allows viewing, toggling mask, testing, and saving Gemini API keys directly from the UI.
    """
    ...
    def __init__(self, parent=None):
        ...
        # Header
        hdr_row = QHBoxLayout()
        hdr_lbl = QLabel("🔑 GEMINI API KEY / เปลี่ยน API KEY")
        hdr_lbl.setFont(QFont(UI_FONT, 8, QFont.Weight.Bold))
        hdr_lbl.setStyleSheet(f"color: {C.PRI}; background: transparent; border: none; letter-spacing: 1px;")
        hdr_row.addWidget(hdr_lbl)
        hdr_row.addStretch()
        ...
        self._status_chip.setFont(QFont("Courier New", 7, QFont.Weight.Bold))
        self._status_chip.setStyleSheet(f"color: {C.GREEN}; background: transparent; border: none;")
        hdr_row.addWidget(self._status_chip)
        lay.addLayout(hdr_row)
        ...
        self._key_input.setPlaceholderText("ใส่ Gemini API Key (AIzaSy...)")
        self._key_input.setFont(QFont("Courier New", 8))
        ...
        self._toggle_btn = QPushButton("👁️")
        self._toggle_btn.setToolTip("แสดง/ซ่อน API Key")
        ...
        # Action Buttons row: Save & Verify
        btn_row = QHBoxLayout()
        btn_row.setSpacing(6)

        self._save_btn = QPushButton("💾 บันทึก API Key")
        self._save_btn.setFont(QFont(UI_FONT, 8, QFont.Weight.Bold))
        ...
        btn_row.addWidget(self._save_btn, stretch=1)

        self._verify_btn = QPushButton("⚡ ตรวจสอบ")
        self._verify_btn.setFont(QFont(UI_FONT, 8))
        self._verify_btn.setFixedHeight(26)
        ...
        self._msg_lbl.setFont(QFont(UI_FONT, 8))
        self._msg_lbl.setWordWrap(True)
        self._msg_lbl.setStyleSheet("background: transparent; border: none;")
        self._msg_lbl.hide()
    ...
    def _load_current_key(self):
        ...
        if not curr_key:
            try:
                ...
                curr_key = load_api_keys().get("gemini_api_key", "")
            except Exception:
                pass
        if curr_key:
            self._key_input.setText(curr_key)
            self._status_chip.setText("ACTIVE")
            self._status_chip.setStyleSheet(f"color: {C.GREEN}; background: transparent; border: none;")
    ...
    def _verify_key(self):
        ...
        if not raw_key:
            self._show_msg("⚠️ กรุณากรอก API Key ก่อนทำการตรวจสอบ", is_error=True)
            return

        from core.api_key_validator import normalize_gemini_api_key, validate_gemini_api_key
        norm_key = normalize_gemini_api_key(raw_key)
        if not norm_key:
            self._show_msg("❌ รูปแบบ API Key ไม่ถูกต้อง", is_error=True)
            return

        self._show_msg("🔄 กำลังตรวจสอบ API Key...", is_error=False)
        QApplication.processEvents()

        res = validate_gemini_api_key(norm_key)
        if res.valid:
            self._status_chip.setText("VALID")
            self._status_chip.setStyleSheet(f"color: {C.GREEN}; background: transparent; border: none;")
            self._show_msg(f"✅ API Key ใช้งานได้สมบูรณ์ ({res.message})", is_error=False)
        else:
            self._status_chip.setText("INVALID")
            self._status_chip.setStyleSheet(f"color: {C.RED}; background: transparent; border: none;")
            self._show_msg(f"❌ {res.message}", is_error=True)

    def _save_key(self):
        raw_key = self._key_input.text().strip()
        if not raw_key:
            self._show_msg("⚠️ กรุณากรอก API Key ก่อนบันทึก", is_error=True)
            return
        ...
        self._show_msg("🔄 กำลังตรวจสอบและบันทึก API Key...", is_error=False)
        QApplication.processEvents()
        ...
        if not res.valid:
            self._status_chip.setText("INVALID")
            self._status_chip.setStyleSheet(f"color: {C.RED}; background: transparent; border: none;")
            self._show_msg(f"❌ ไม่สามารถบันทึกได้: {res.message}", is_error=True)
            return

        # Persist key
        os.environ["GEMINI_API_KEY"] = norm_key
        try:
            from memory.config_manager import save_api_keys
            save_api_keys(norm_key)
        except Exception:
            pass
        ...
        try:
            ...
            store.set("gemini_api_key", norm_key)
        except Exception:
            pass

        self._status_chip.setText("SAVED")
        self._status_chip.setStyleSheet(f"color: {C.GREEN}; background: transparent; border: none;")
        self._show_msg("✅ บันทึก API Key สำเร็จแล้ว!", is_error=False)
        self.api_key_changed.emit(norm_key)

    def _show_msg(self, text: str, is_error: bool = False):
        self._msg_lbl.setText(text)
        color = C.RED if is_error else C.GREEN
        self._msg_lbl.setStyleSheet(f"color: {color}; background: transparent; border: none;")
        self._msg_lbl.show()


# ---------------------------------------------------------------------------
# LiveActivitySidebarWidget — sidebar live activity & status panel
# ---------------------------------------------------------------------------

class LiveActivitySidebarWidget(QFrame):
    """Left Sidebar Live Activity Monitor.
    Displays real-time status (LISTENING / THINKING / WORKING / SPEAKING),
    the exact current step/action being performed, active tool execution details,
    and a live step-by-step history log.
    """
    ...
    def __init__(self, parent=None):
        ...
        hdr = QLabel("⚡ LIVE STATUS  /  สถานะปัจจุบัน")
        hdr.setFont(QFont(UI_FONT, 8, QFont.Weight.Bold))
        hdr.setStyleSheet(f"color: {C.PRI}; background: transparent; border: none; letter-spacing: 1px;")
        lay.addWidget(hdr)
        ...
        self._dot = QLabel("●")
        self._dot.setFont(QFont("Segoe UI", 10, QFont.Weight.Bold))
        self._dot.setStyleSheet(f"color: {C.GREEN}; background: transparent; border: none;")
        status_row.addWidget(self._dot)

        self._state_lbl = QLabel("ONLINE / พร้อมรับคำสั่ง")
        self._state_lbl.setFont(QFont(UI_FONT, 8, QFont.Weight.DemiBold))
        self._state_lbl.setStyleSheet(f"color: {C.WHITE}; background: transparent; border: none;")
        status_row.addWidget(self._state_lbl, stretch=1)

        lay.addLayout(status_row)

        # Current action description
        self._action_lbl = QLabel("รอรับฟังคำสั่งจากผู้ใช้...")
        self._action_lbl.setFont(QFont(UI_FONT, 8))
        ...
        # Step history view
        self._step_box = QTextEdit()
        self._step_box.setReadOnly(True)
        self._step_box.setFixedHeight(75)
        self._step_box.setFont(QFont("Courier New", 7))
    ...
    def update_state(self, state: str):
        color_map = {
            "LISTENING": (C.GREEN, "🎙️ LISTENING / กำลังฟัง...", "กำลังรับฟังเสียงพูดของคุณ..."),
            "THINKING": (C.ACC2, "🧠 THINKING / กำลังประมวลผล...", "กำลังคิดและวิเคราะห์คำตอบ..."),
            "PROCESSING": (C.ACC2, "⚙️ PROCESSING / กำลังประมวลผล...", "กำลังประมวลผลขั้นตอน..."),
            "SPEAKING": (C.PRI, "🔊 SPEAKING / กำลังพูดโต้ตอบ...", "กำลังสตรีมเสียงพูดตอบกลับ..."),
            "MUTED": (C.RED, "🔇 MUTED / ปิดไมโครโฟน", "ไมโครโฟนถูกปิดอยู่"),
        }
        color, title, desc = color_map.get(state, (C.PRI, "⚡ ONLINE / พร้อมใช้งาน", "พร้อมรับคำสั่งใหม่..."))
        self._dot.setStyleSheet(f"color: {color}; background: transparent; border: none;")
        self._state_lbl.setText(title)
        self._action_lbl.setText(desc)
        self.append_step(f"[{state}] {desc}")

    def update_tool(self, tool_name: str, status: str = "started"):
        if status == "started":
            self._dot.setStyleSheet(f"color: {C.PURPLE}; background: transparent; border: none;")
            self._state_lbl.setText(f"⚡ WORKING / {tool_name.upper()}")
            self._action_lbl.setText(f"กำลังเรียกใช้งานเครื่องมือ {tool_name}...")
            self.append_step(f"[TOOL] 🔍 เรียกใช้ {tool_name}")
        elif status == "finished":
            self.append_step(f"[TOOL] ✅ {tool_name} สำเร็จ")

    def append_step(self, msg: str):
        if not msg:
            return
        ts = time.strftime("%H:%M:%S")
        self._step_box.append(f"[{ts}] {msg}")
        sb = self._step_box.verticalScrollBar()
        if sb:
            sb.setValue(sb.maximum())
...
class MainWindow(QMainWindow):
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        # ── Live Activity Sidebar Card ──────────────────────────────────────
        self._live_sidebar = LiveActivitySidebarWidget()
        lay.addWidget(self._live_sidebar)

        # ── Context Token Meter Card ────────────────────────────────────────
        self._token_meter = TokenMeterWidget()
        lay.addWidget(self._token_meter)

        # ── Gemini API Key Card ──────────────────────────────────────────────
        self._api_key_widget = GeminiApiKeyWidget()
        lay.addWidget(self._api_key_widget)
    ...
    def _apply_state(self, state: str):
        ...
        if hasattr(self, "_tool_progress"):
            self._tool_progress.set_state_status(state)

        if hasattr(self, "_live_sidebar"):
            self._live_sidebar.update_state(state)
    ...
    def _parse_log_for_context(self, text: str):
        ...
        if hasattr(self, "_tool_progress"):
            self._tool_progress.append_activity_log(text)

        if hasattr(self, "_live_sidebar"):
            self._live_sidebar.append_step(text)
...
class JarvisUI:
    ...
    def update_token_usage(self, used_tokens: int, max_tokens: int = 1048576):
        """Thread-safe update of token usage gauge."""
        try:
            if hasattr(self._win, "_token_meter"):
                self._win._token_meter.set_token_usage(used_tokens, max_tokens)
            if hasattr(self._win, "_tool_progress") and hasattr(self._win._tool_progress, "_token_chip_lbl"):
                rem = max(0, max_tokens - used_tokens)
                if rem >= 1000000:
                    text = f"🎯 {rem / 1000000:.2f}M TOKENS"
                else:
                    text = f"🎯 {rem:,} TOKENS"
                self._win._tool_progress._token_chip_lbl.setText(text)
        except Exception:
            pass
