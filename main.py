...
class JarvisLive:
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        if len(clean_result) > 600:
            clean_result = clean_result[:600] + "... [truncated for live audio speed]"
        return types.FunctionResponse(
            id=fc.id, name=name,
    ...
    async def _send_realtime(self):
        try:
            ...
            if not self._shutdown_requested.is_set():
                print(f"[JARVIS] 🔌 Realtime input stream ended ({type(e).__name__}: {e})")
                ...
                async for response in self.session.receive():
                    ...
                    if response.server_content:
                        ...
                        if sc.turn_complete:
                            ...
                            # Update real-time token usage gauge
                            full_in_tokens = max(1, len(full_in) // 3) if full_in else 0
                            full_out_tokens = max(1, len(full_out) // 3) if full_out else 0
                            self._total_tokens_used = getattr(self, "_total_tokens_used", 2850) + full_in_tokens + full_out_tokens + 25
                            if hasattr(self.ui, "update_token_usage"):
                                self.ui.update_token_usage(self._total_tokens_used)
                            if (
                                getattr(self, "_pending_self_quit", False)
                    ...
                    if response.tool_call:
                        ...
                        try:
                            ...
                            print(f"[JARVIS] ⚠️ Tool response send failed: {te}")
                    if self._shutdown_requested.is_set():
                        return
        except Exception as e:
            if self._shutdown_requested.is_set():
                return
            err_str = str(e).lower()
            if "1000" in str(e) or "connectionclosed" in type(e).__name__.lower() or "closed" in err_str:
                print(f"[JARVIS] 🔌 Session closed ({type(e).__name__}: {e}).")
                return
